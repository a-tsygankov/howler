...
| **2.8 — Design-system port** ✅ *(desktop view + visual-regression tests still queued in 2.8c)* | Visual rewrite | **2.8 core** ✅ Tailwind 3.4 + warm-domestic palette ported verbatim from `docs/design_handoff_howler/styles.css` into `tailwind.config.ts`; Google Fonts (Fraunces / Source Serif 4 / Inter Tight / JetBrains Mono); primitives `HowlerAvatar` (Option B + CVD glyph redundancy), `ProgressBar`, `SegBtn`, `Sheet`, `ResultSlider`, `Btn`; Day Ribbon home with By time / By label seg toggle (localStorage-persisted); Slider Sheet ack flow (variant B); Login re-skinned. **2.8b** ✅ react-router-dom routing; Icon Set A barrel ([webapp/src/components/Icon.tsx](webapp/src/components/Icon.tsx)) with 25 names; Task detail page with hand-rolled SVG sparkline over `task_executions`; Result-types manager screen (full CRUD with delete-warning per §6.3 #10); new `GET /api/tasks/:id/executions` + `PATCH /api/task-results/:id` endpoints. **2.8c queued:** desktop view (1200×760) + visual-regression Playwright screenshots. `data-testid` contract preserved across redesign + new screens. |
