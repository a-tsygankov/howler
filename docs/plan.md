...
| **3 — Web stability gate** 🟡 *(visual regression deferred to 2.8)* | Beta-ready | **3.1** ✅ Playwright happy paths (quick-setup → create → see task; login screen renders; API health) running against the deployed Pages preview. **3.2** ✅ CSP + HSTS + frame-deny + permissions-policy on every Pages response (`webapp/functions/_middleware.ts`). **3.3** ✅ Structured JSON logs (`backend/src/logger.ts`) on every request + cron + error path, ready for Logpush. **3.4** ✅ SLO doc + Logpush setup recipe in `docs/observability.md` (cron lag p99 < 90 s, ack p99 < 500 ms, auth error < 5 %, push > 95 %, 5xx < 0.5 %, p50 latency < 250 ms). **3.5** ✅ CI runs Playwright on every webapp PR. **2.8 visual regression deferred** — current visual layer is placeholder; pixel-fidelity tests land alongside the design-system port. **Gate to Phase 4: 7-day stability watch on `main` + two non-engineer testers complete the happy path.** |
