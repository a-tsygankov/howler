...
| **2.8 — Design-system port** ✅ *(visual-regression baselines deferred until the bottom-tab nav lands; current visual layer is still pre-tab-bar and would need re-seeding the moment that ships)* | Visual rewrite | **2.8 core** ✅ Tailwind 3.4 + warm-domestic palette ported verbatim from `docs/design_handoff_howler/styles.css` into `tailwind.config.ts`; Google Fonts; primitives `HowlerAvatar` (Option B + CVD glyph redundancy), `ProgressBar`, `SegBtn`, `Sheet`, `ResultSlider`, `Btn`; Day Ribbon home; Slider Sheet ack flow; Login re-skinned. **2.8b** ✅ react-router routing; Icon Set A barrel; Task detail + sparkline; Result-types manager; `GET /api/tasks/:id/executions` + `PATCH /api/task-results/:id`. **2.8c** ✅ Desktop sidebar (`webapp/src/components/Sidebar.tsx`, hidden on mobile via `hidden lg:flex`, shows Today / Result types nav + per-home Labels list); Dashboard / TaskDetail / ResultTypesManager widen from `max-w-md` to `lg:max-w-2xl`; new opt-in Playwright `visual` project (`webapp/e2e/visual.spec.ts`) with three baseline points (login mobile / login desktop / dashboard desktop with sidebar) — disabled in default `pnpm test:e2e` and CI until baselines are seeded from a canonical Linux Chromium via `pnpm test:e2e:visual:update`. |
| **3 — Web stability gate** ✅ *(visual regression deferred to 2.8 alongside bottom-tab nav)* | Beta-ready | **3.1** ✅ Playwright happy paths (quick-setup → create → see task; login screen renders; API health) running against the deployed Pages preview. **3.2** ✅ CSP + HSTS + frame-deny + permissions-policy on every Pages response (`webapp/functions/_middleware.ts`). **3.3** ✅ Structured JSON logs (`backend/src/logger.ts`) on every request + cron + error path, ready for Logpush. **3.4** ✅ SLO doc + Logpush setup recipe in `docs/observability.md` (cron lag p99 < 90 s, ack p99 < 500 ms, auth error < 5 %, push > 95 %, 5xx < 0.5 %, p50 latency < 250 ms). **3.5** ✅ CI runs Playwright on every webapp PR. **3.6** ✅ Synthetic 7-day stability harness (`backend/test/stability-7day.test.ts`) drives a DAILY 3×/day task across 168 simulated hours via `TestClock` and asserts 21 fires, no orphan re-fires, advancing `next_fire_at`, ack idempotency. **3.7** ✅ Two concurrent Playwright contexts (`webapp/e2e/two-user.spec.ts`) drive quick-setup → create → see-own-task in parallel, plus an API-level cross-home isolation check. Together 3.6 + 3.7 substitute for the original "7-day watch + two non-engineer testers" gate. |
