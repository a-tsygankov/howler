...
| **2 — Server + Web hardening** ✅ *(2.6b deferred)* | Feature-completion + UX polish | **2.0 — home-centric model rework** ✅ (migration 0002): HOME / USER / LABEL / TASK_ASSIGNMENT / TASK_RESULT / TASK_EXECUTION; tokens carry `homeId`; user-picker step. **2.1 — users CRUD** ✅ (add/rename/remove with private-task tombstone cleanup). **2.2 — devices list + revoke** ✅. **2.3 — rate-limiting** ✅ (CF binding on /login, /login-qr, /setup, /quick-setup, /pair/confirm). **2.4 — schedule templates** ✅ (migration 0003; 5 system templates seeded per home; tasks accept `templateId`). **2.5 — Option B avatars** ✅ (migration 0004; R2-backed POST /api/avatars; PATCH endpoints accept `avatarId`; webapp uploads home avatar). **2.6 — web push (plumbing)** ✅ — *encryption deferred to **2.6b***: migration 0005, push_subscriptions table, /api/push endpoints, service worker, permission UX. Browsers will silently drop pushes until 2.6b ships the VAPID-signed encrypted payloads. **2.7 — Workers Analytics dashboards** ⛔ deferred (separate observability chunk). |
