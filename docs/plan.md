...
| **2.8 — Design-system port** 🟍 *(queued)* | Visual rewrite | Tailwind + shadcn/ui in webapp; tokens from `docs/design_handoff_howler/styles.css` ported to `tailwind.config.ts`; Google Fonts (Fraunces / Source Serif 4 / Inter Tight / JetBrains Mono); primitives `<HowlerAvatar>` (Option B + CVD redundancy), `<ProgressBar>`, `<SegBtn>`, `<BottomTabs>`, `<Sheet>`; Day Ribbon home screen; Slider Sheet ack flow; task list/detail/sparkline; result-types manager; icon Set A barrel; settings/users/devices re-skinned in the `<Phone>` framing; thin desktop baseline. Visual regression tests (Playwright screenshots) land with this phase since stable selectors live alongside the redesign. |
