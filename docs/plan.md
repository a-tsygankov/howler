...
| **2.8 — Design-system port** ✅ *(core; task-detail/sparkline + icon Set A + desktop view deferred)* | Visual rewrite | Tailwind 3.4 + warm-domestic palette ported verbatim from `docs/design_handoff_howler/styles.css` into `tailwind.config.ts`; Google Fonts (Fraunces / Source Serif 4 / Inter Tight / JetBrains Mono) wired in `src/styles.css`; primitives `HowlerAvatar` (Option B + CVD glyph redundancy), `ProgressBar`, `SegBtn`, `Sheet`, `ResultSlider`, `Btn`; Day Ribbon home with By time / By label seg toggle (localStorage-persisted); Slider Sheet ack flow (variant B); Login re-skinned. `data-testid` contract preserved end-to-end so the Phase 3 Playwright suite still passes (3/3 vs dev-5 preview). Task detail + sparkline, the result-types manager screen, the icon Set A barrel, and the desktop baseline are deferred — captured as 2.8b in the punchlist. Visual-regression tests will land alongside 2.8b. |
