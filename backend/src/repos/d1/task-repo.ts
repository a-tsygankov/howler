  ...
  async findMany(spec: ISpecification<Task>): Promise<Task[]> {
    if (spec.tag === "OwnedBy") {
      ...
      const { results } = await this.d1
        .prepare(
          "SELECT * FROM tasks WHERE user_id = ? AND is_deleted = 0 ORDER BY updated_at DESC",
        )
        .bind(userId)
        .all<TaskRow>();
  ...
  add(t: Task): Promise<void> {
    this.uow.enqueue(
      this.d1
        .prepare(
          `INSERT INTO tasks
           (id, user_id, title, description, priority, kind, deadline_hint, avatar_id,
      ...
      this.d1
        .prepare(
          `UPDATE tasks SET
             ...
             deadline_hint = ?, avatar_id = ?, active = ?,
