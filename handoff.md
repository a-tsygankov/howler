...
**Last updated:** 2026-06-03 — through the `feat/device-rename-task-targeting` branch (not yet a PR). Recent arcs: **OTA Phase 6** end-to-end except HIL-3 hardware verification; **avatars** unified across home / users / tasks / labels on all three layers; **device power**: 5-min idle screen-sleep with peek-skip wake; **device rename + task targeting** (new, below). Tests at backend 152 + firmware native 127.

**Device rename + task user/device targeting (`feat/device-rename-task-targeting`).** Spec: [`docs/superpowers/specs/2026-06-02-device-rename-task-targeting-design.md`](docs/superpowers/specs/2026-06-02-device-rename-task-targeting-design.md). Migration `0017` adds `devices.name` + `task_device_assignments` (+ counter triggers: assignment ins/del, device insert, device name-only update — scoped so `last_seen_at` writes don't churn). A task now targets **either** users (private to assignee + creator + admin) **or** devices (shared, shown on those dials' Today) — exclusive, enforced in the task service; `is_private` is kept in sync but the assignment tables are authoritative. A shared visibility predicate (`backend/src/services/visibility.ts`) gates every task read + mutation (dashboard, tasks list/get/patch/delete/complete/executions, occurrences pending/ack); dashboard rows carry `assignedToThisDevice` + `assignedUserIds`/`assignedDeviceIds`. New endpoints: `PATCH /api/devices/:id` (rename, bumps counter) + `GET /api/devices/me` (dial self-identity). Firmware: the device-token dashboard split routes only `assignedToThisDevice` rows to Today (All keeps every shared task); a new `DeviceIdentity` (fetched via `/api/devices/me`, threaded through `SyncService` next to `HomeIdentity`) shows the dial's user-set name on the About card. Webapp: Settings → Devices inline rename; a 3-mode Everyone/Users/Devices picker replacing the single assignee select + Private checkbox in the create form + All-tasks editor; "created by" on task rows + TaskDetail. **Note:** existing user-assigned tasks become genuinely private now (visibility was stored but never enforced before). **Pending apply:** migration `0017` needs `wrangler d1 migrations apply howler-db --remote` (runs on next main merge with the workflow green, like the others below).

**Device idle / wake-skip sync (#49, dev-46).** New `Settings → Sleep` picker (Off / 5 / 10 / 15 / 30 min, default 5). At idle: backlight off via `digitalWrite(TFT_BL, LOW)`, LED ring muted via `App::setUiIdle(true)`, `SyncService::tick()` paused. On any touch / knob: backlight restored, the first wake-event is swallowed (mobile UX convention), `SyncService::requestPeekNow()` schedules an immediate peek. The peek-then-skip path now works across reboots — `SyncService` persists `lastCounter_` to NVS at every counter update and restores it (alongside anchoring `lastFullRoundMs_` to "now") in `App::begin()`. On every wake / cold boot, if the home counter is unchanged the device shows the previous-session dashboard without a single fetch. Also bumped `playDoneAnimation` default 900 → 500 ms (snappier completion check). True deep-sleep is a follow-up — current screen-sleep keeps Wi-Fi up for queued mark-done drain. Counter-trigger coverage (migration `0012_update_counter.sql`) re-audited: every user-visible mutation on tasks / users / home / labels / task_results / avatars / occurrences / task_assignments / schedules bumps; cron-fanout schedule updates (next_fire_at / updated_at only) deliberately skip the bump to avoid per-minute churn.

**CI version-bump check (#49).** New `scripts/check_version_bump.py` + `.github/workflows/version-check.yml`. PRs touching `webapp/` / `firmware/` / `backend/` (non-migration) / `backend/migrations/` must bump the corresponding version source (`package.json` `.version` for webapp & worker; `Version.h kFirmwareVersion` for firmware; new numbered `.sql` for schema). Doc-only PRs are exempt. Firmware bumped to 0.3.1 in this PR.
...
1. **Apply pending migrations to remote D1.** `0012_update_counter.sql` (slice A), `0013_firmware_releases.sql` (Phase 6), `0014_user_admin.sql` (per-user admin), `0015_label_avatars.sql` (labels), `0016_avatar_1bit.sql` (Phase 7), and `0017_device_name_and_assignment.sql` (device rename + task targeting) all need `pnpm dlx wrangler d1 migrations apply howler-db --remote` from `backend/`. The `deploy.yml` workflow runs this on every main merge, so each migration applies the next time someone merges with the workflow green.
