...
**Last updated:** 2026-05-10 — through PR #41 (OTA admin UI). PR #42 (avatars on labels + device home identity) in flight. The whole OTA Phase 6 arc shipped over 7 PRs; the avatar surface is now consistent across home / users / tasks / labels on backend + webapp + device.
...
**Avatar functionality across the stack — three PRs.** **#40** activated photo upload in webapp for users + tasks + home, added `AvatarUploadButton` component, and rendered user avatars on the device's UserPicker via `RoundMenu` centre-slot badges. **#42** (in flight) extends to labels (migration 0015 adds `labels.avatar_id`, unifies on `'icon:<name>'` / UUID format), adds `GET /api/homes/me` so the device can fetch home identity (replaces hardcoded "Howler" on Settings → About with the actual home name + avatar). UUID photo avatars fall back to text initials on the device — coherent device-wide stance since the 240×240 1-bit-bitmap renderer can't display arbitrary photos.
...
1. **Apply pending migrations to remote D1.** `0012_update_counter.sql` (slice A), `0013_firmware_releases.sql` (Phase 6 foundation), `0014_user_admin.sql` (per-user admin), and `0015_label_avatars.sql` (label avatars, in flight on PR #42) all need `pnpm dlx wrangler d1 migrations apply howler-db --remote` from `backend/`. The `deploy.yml` workflow runs this on every main merge, so each migration applies the next time someone merges to main with the workflow green.
...
4. **MQTT bridge** (Phase 5 deferred). Non-Cloudflare component (HiveMQ Cloud / AWS IoT / self-hosted Mosquitto) — pick a broker, write the bridge service, swap in an `MqttNetwork` adapter behind a feature flag. The `INetwork` abstraction means the device side is a one-adapter change.
5. **Visual regression baselines** (deferred from Phase 2.8). The bottom-tab nav landed in dev-14, so the gate is met; needs a canonical Linux Chromium run to seed the snapshots.
...
7. **AI conversion of photo avatars to 24×24 1-bit bitmaps** (Phase 7). Currently UUID photo avatars fall back to text initials on the device — coherent but visually less rich. A Worker-side image processor could pre-compute the 1-bit version on upload and serve it via `/api/avatars/:id?format=1bit` so the dial picks up the photo (downsampled to glyph-style) the same way it picks up icon presets.
